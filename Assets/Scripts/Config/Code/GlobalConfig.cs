    ...
    public partial struct GlobalConfig
    {
        public static void DeserializeByAddressable(string directory)
        {
            ...
            for (int i = 0; i < array.Count; i++)
            {
                ...
                GlobalConfig data = (GlobalConfig)dataObject.ToObject(typeof(GlobalConfig));
                datas.Add(data);
        ...
        {
            string bundleName = $"{subFolder}/GlobalConfig.bytes".ToLower();
        ...
        private static List<GlobalConfig> datas;
        private static Dictionary<int, int> indexMap;
        public static GlobalConfig ByID(int id)
        {
            if (id <= 0)
            {
                return Null;
            ...
            {
                throw new System.Exception($"GlobalConfig找不到ID:{id}");
            }
            return ByIndex(index);
        }
        public static GlobalConfig ByIndex(int index)
        {
            return datas[index];
        }
        public bool IsNull { get; private set; }
        public static GlobalConfig Null { get; } = new GlobalConfig() { IsNull = true }; 
        public System.Int32 ID { get; set; }
        public string Description { get; set; }
        public string Name { get; set; }
        public System.Single FloatValue { get; set; }
