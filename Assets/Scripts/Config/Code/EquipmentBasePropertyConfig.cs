using Newtonsoft.Json.Linq;
using System.Collections.Generic;
using UnityEngine.AddressableAssets;
namespace Config
{
    public partial struct EquipmentBasePropertyConfig
    {
        public static void DeserializeByAddressable(string directory)
        {
            string path = $"{directory}/EquipmentBasePropertyConfig.json";
            UnityEngine.TextAsset ta = Addressables.LoadAssetAsync<UnityEngine.TextAsset>(path).WaitForCompletion();
            string json = ta.text;
            datas = new List<EquipmentBasePropertyConfig>();
            indexMap = new Dictionary<int, int>();
            JArray array = JArray.Parse(json);
            Count = array.Count;
            for (int i = 0; i < array.Count; i++)
            {
                JObject dataObject = array[i] as JObject;
                EquipmentBasePropertyConfig data = (EquipmentBasePropertyConfig)dataObject.ToObject(typeof(EquipmentBasePropertyConfig));
                datas.Add(data);
                indexMap.Add(data.ID, i);
            }
        }
        public static void DeserializeByFile(string directory)
        {
            string path = $"{directory}/EquipmentBasePropertyConfig.json";
            using (System.IO.FileStream fs = new System.IO.FileStream(path, System.IO.FileMode.Open, System.IO.FileAccess.Read))
            {
                using (System.IO.StreamReader reader = new System.IO.StreamReader(fs))
                {
                    datas = new List<EquipmentBasePropertyConfig>();
                    indexMap = new Dictionary<int, int>();
                    string json = reader.ReadToEnd();
                    JArray array = JArray.Parse(json);
                    Count = array.Count;
                    for (int i = 0; i < array.Count; i++)
                    {
                        JObject dataObject = array[i] as JObject;
                        EquipmentBasePropertyConfig data = (EquipmentBasePropertyConfig)dataObject.ToObject(typeof(EquipmentBasePropertyConfig));
                        datas.Add(data);
                        indexMap.Add(data.ID, i);
                    }
                }
            }
        }
        public static System.Collections.IEnumerator DeserializeByBundle(string directory, string subFolder)
        {
            string bundleName = $"{subFolder}/EquipmentBasePropertyConfig.bytes".ToLower();
            string fullBundleName = $"{directory}/{bundleName}";
            string assetName = $"assets/{bundleName}";
            #if UNITY_WEBGL && !UNITY_EDITOR
            UnityEngine.AssetBundle bundle = null;
            UnityEngine.Networking.UnityWebRequest request = UnityEngine.Networking.UnityWebRequestAssetBundle.GetAssetBundle(fullBundleName);
            yield return request.SendWebRequest();
            if (request.isNetworkError || request.isHttpError)
            {
                UnityEngine.Debug.LogError(request.error);
            }
            else
            {
                bundle = UnityEngine.Networking.DownloadHandlerAssetBundle.GetContent(request);
            }
            #else
            yield return null;
            UnityEngine.AssetBundle bundle = UnityEngine.AssetBundle.LoadFromFile($"{fullBundleName}", 0, 0);
            #endif
            UnityEngine.TextAsset ta = bundle.LoadAsset<UnityEngine.TextAsset>($"{assetName}");
            string json = ta.text;
            datas = new List<EquipmentBasePropertyConfig>();
            indexMap = new Dictionary<int, int>();
            JArray array = JArray.Parse(json);
            Count = array.Count;
            for (int i = 0; i < array.Count; i++)
            {
                JObject dataObject = array[i] as JObject;
                EquipmentBasePropertyConfig data = (EquipmentBasePropertyConfig)dataObject.ToObject(typeof(EquipmentBasePropertyConfig));
                datas.Add(data);
                indexMap.Add(data.ID, i);
            }
        }
        public static int Count;
        private static List<EquipmentBasePropertyConfig> datas;
        private static Dictionary<int, int> indexMap;
        public static EquipmentBasePropertyConfig ByID(int id)
        {
            if (id <= 0)
            {
                return Null;
            }
            if (!indexMap.TryGetValue(id, out int index))
            {
                throw new System.Exception($"EquipmentBasePropertyConfig找不到ID:{id}");
            }
            return ByIndex(index);
        }
        public static EquipmentBasePropertyConfig ByIndex(int index)
        {
            return datas[index];
        }
        public bool IsNull { get; private set; }
        public static EquipmentBasePropertyConfig Null { get; } = new EquipmentBasePropertyConfig() { IsNull = true }; 
        public System.Int32 ID { get; set; }
        public string Description { get; set; }
        public ItemType ItemType { get; set; }
        public Jobs Jobs { get; set; }
        public Parts Parts { get; set; }
        public System.Int32 LimitLevel { get; set; }
        public System.Int32[] LimitAttribute { get; set; }
        public RandomAttributeData[] RandomAttributeDatas { get; set; }
        public System.Int32 AttributeGroupId { get; set; }
        public System.Int32 MaxGemSlot { get; set; }
        public System.Single AttackSpeed { get; set; }
        public string Icon { get; set; }
        public string EquipDescribe { get; set; }
        public string EquipmentShowModel { get; set; }
        public string EquipmentHighQuality { get; set; }
        public string EquipmentDropModel { get; set; }
        public System.Int32[] EquipmentQualityInterval { get; set; }
    }
}
