    ...
    public partial struct DungeonMapConfig
    {
        public static void DeserializeByAddressable(string directory)
        {
            ...
            for (int i = 0; i < array.Count; i++)
            {
                ...
                DungeonMapConfig data = (DungeonMapConfig)dataObject.ToObject(typeof(DungeonMapConfig));
                datas.Add(data);
        ...
        {
            string bundleName = $"{subFolder}/DungeonMapConfig.bytes".ToLower();
        ...
        private static List<DungeonMapConfig> datas;
        private static Dictionary<int, int> indexMap;
        public static DungeonMapConfig ByID(int id)
        {
            if (id <= 0)
            {
                return Null;
            ...
            {
                throw new System.Exception($"DungeonMapConfig找不到ID:{id}");
            }
            return ByIndex(index);
        }
        public static DungeonMapConfig ByIndex(int index)
        {
            return datas[index];
        }
        public bool IsNull { get; private set; }
        public static DungeonMapConfig Null { get; } = new DungeonMapConfig() { IsNull = true }; 
