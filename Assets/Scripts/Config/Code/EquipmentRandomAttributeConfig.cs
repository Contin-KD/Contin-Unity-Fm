    ...
    public partial struct EquipmentRandomAttributeConfig
    {
        public static void DeserializeByAddressable(string directory)
        {
            ...
            for (int i = 0; i < array.Count; i++)
            {
                ...
                EquipmentRandomAttributeConfig data = (EquipmentRandomAttributeConfig)dataObject.ToObject(typeof(EquipmentRandomAttributeConfig));
                datas.Add(data);
        ...
        {
            string bundleName = $"{subFolder}/EquipmentRandomAttributeConfig.bytes".ToLower();
        ...
        private static List<EquipmentRandomAttributeConfig> datas;
        private static Dictionary<int, int> indexMap;
        public static EquipmentRandomAttributeConfig ByID(int id)
        {
            if (id <= 0)
            {
                return Null;
            ...
            {
                throw new System.Exception($"EquipmentRandomAttributeConfig找不到ID:{id}");
            }
            return ByIndex(index);
        }
        public static EquipmentRandomAttributeConfig ByIndex(int index)
        {
            return datas[index];
        }
        public bool IsNull { get; private set; }
        public static EquipmentRandomAttributeConfig Null { get; } = new EquipmentRandomAttributeConfig() { IsNull = true }; 
