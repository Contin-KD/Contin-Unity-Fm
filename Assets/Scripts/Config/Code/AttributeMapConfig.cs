    ...
    public partial struct AttributeMapConfig
    {
        public static void DeserializeByAddressable(string directory)
        {
            ...
            for (int i = 0; i < array.Count; i++)
            {
                ...
                AttributeMapConfig data = (AttributeMapConfig)dataObject.ToObject(typeof(AttributeMapConfig));
                datas.Add(data);
        ...
        {
            string bundleName = $"{subFolder}/AttributeMapConfig.bytes".ToLower();
        ...
        private static List<AttributeMapConfig> datas;
        private static Dictionary<int, int> indexMap;
        public static AttributeMapConfig ByID(int id)
        {
            if (id <= 0)
            {
                return Null;
            ...
            {
                throw new System.Exception($"AttributeMapConfig找不到ID:{id}");
            }
            return ByIndex(index);
        }
        public static AttributeMapConfig ByIndex(int index)
        {
            return datas[index];
        }
        public bool IsNull { get; private set; }
        public static AttributeMapConfig Null { get; } = new AttributeMapConfig() { IsNull = true }; 
        public System.Int32 ID { get; set; }
        public string Description { get; set; }
        public AttributeType AttributeType { get; set; }
        public string AttributeDescribe { get; set; }
        public string EnglishMark { get; set; }
        public System.Single LowerLimit { get; set; }
        public System.Single UpperLimit { get; set; }
    }
}
