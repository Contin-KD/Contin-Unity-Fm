    ...
    public partial struct CombatFormulaConfig
    {
        public static void DeserializeByAddressable(string directory)
        {
            ...
            for (int i = 0; i < array.Count; i++)
            {
                ...
                CombatFormulaConfig data = (CombatFormulaConfig)dataObject.ToObject(typeof(CombatFormulaConfig));
                datas.Add(data);
        ...
        {
            string bundleName = $"{subFolder}/CombatFormulaConfig.bytes".ToLower();
        ...
        private static List<CombatFormulaConfig> datas;
        private static Dictionary<int, int> indexMap;
        public static CombatFormulaConfig ByID(int id)
        {
            if (id <= 0)
            {
                return Null;
            ...
            {
                throw new System.Exception($"CombatFormulaConfig找不到ID:{id}");
            }
            return ByIndex(index);
        }
        public static CombatFormulaConfig ByIndex(int index)
        {
            return datas[index];
        }
        public bool IsNull { get; private set; }
        public static CombatFormulaConfig Null { get; } = new CombatFormulaConfig() { IsNull = true }; 
        public System.Int32 ID { get; set; }
        public string Description { get; set; }
        public System.Single CoefficientA { get; set; }
        public System.Single CoefficientB { get; set; }
        public System.Single[] IntervalEvasion { get; set; }
    }
}
