    ...
    public partial struct AudioConfig
    {
        public static void DeserializeByAddressable(string directory)
        {
            ...
            for (int i = 0; i < array.Count; i++)
            {
                ...
                AudioConfig data = (AudioConfig)dataObject.ToObject(typeof(AudioConfig));
                datas.Add(data);
        ...
        {
            string bundleName = $"{subFolder}/AudioConfig.bytes".ToLower();
        ...
        private static List<AudioConfig> datas;
        private static Dictionary<int, int> indexMap;
        public static AudioConfig ByID(int id)
        {
            if (id <= 0)
            {
                return Null;
            ...
            {
                throw new System.Exception($"AudioConfig找不到ID:{id}");
            }
            return ByIndex(index);
        }
        public static AudioConfig ByIndex(int index)
        {
            return datas[index];
        }
        public bool IsNull { get; private set; }
        public static AudioConfig Null { get; } = new AudioConfig() { IsNull = true }; 
